...
| `memory/` | Memory-provider abstraction, mirroring the `pickers/` auto-resolution pattern. `init.lua` resolves a provider (`opts.memory.provider`, or vectorcode-then-claude-mem auto-detection) via each provider's `setup()`/`is_available()`. `vectorcode.lua` shims the existing `history/vectorcode.lua`. `claude_mem.lua` talks to a local [claude-mem](https://github.com/thedotmack/claude-mem) HTTP worker over `curl`: builds the `@memory` tool (keyword/semantic search + id unfold, plus a `scope = "prompts"` mode over `GET /api/search?type=prompts`), pushes generated summaries via `POST /api/memory/save`, and optionally fetches recent context for injection into new chats (`get_context`/`wants_context_injection`). When `claude_mem.prompts.enabled = true`, `History:_maybe_save_prompt` (in `init.lua`, hooked off `CodeCompanionChatSubmitted`) pushes each submitted user prompt via `save_prompt` → `POST /api/import` (not `/api/sessions/init`, which would also spawn claude-mem's own LLM observer agent); prompt text is sanitised and length-capped in Lua since `/api/import` applies neither. When `claude_mem.auto_start_worker = true`, a failed request triggers one auto-start attempt per session (`_resolve_start_command`/`_start_worker`): a global `claude-mem` binary on PATH first, else the newest non-orphaned plugin version under the Claude Code plugin cache, invoked via `node bun-runner.js worker-service.cjs start`. |
...
| `test_memory_providers.lua` | Memory-provider auto-resolution (`memory/init.lua`), extension wiring (tool registration, repeat-`setup()` idempotency), context-injection helper (`History:_maybe_inject_memory_context`), prompt-capture helper (`History:_maybe_save_prompt`) |
| `test_memory_claude_mem.lua` | claude-mem provider: config resolution, `is_available()`, `@memory` tool (search/unfold/prompts-scope/error paths), `index()` save/backfill, `get_context()`, `save_prompt()` (import payload, privacy stripping, truncation, `wants_prompt_capture()`) |
...
| `test_utils.lua` | `message_text`, `format_adapter_error`, `format_relative_time`, `visible_user_prompts` normalisation helpers |
